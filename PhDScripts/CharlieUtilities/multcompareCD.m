function [comparison,means,h,gnames] = multcompareCD(stats,varargin)
...
while(~isempty(onetype))
   ...
   switch onetype
    ...
    case 'dunn-sidak'
     ...
     tstatCD = tcdf(-abs(t),df);
     if min(tstatCD)>10e-7
        pval = 1 - (1-2*tstatCD).^kstar;
     else
         pval = nan(size(tstatCD));
         for cd = 1:numel(pval)
             tst =sym(tstatCD(cd),'f');
             pval(cd) = double(1 - (1-2*tst).^kstar);
