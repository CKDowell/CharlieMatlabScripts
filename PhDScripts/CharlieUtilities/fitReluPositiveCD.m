function params = fitReluPositiveCD(x,y)
...
[x,xi] = sort(x,'ascend');
y = y(xi);
% fit in ascending direction - needs positive slope
...
% fit in descending direction - needs negative slope
[x,xi] = sort(x,'descend');
y = y(xi);
mse_desc = inf;
for i = 1:numel(x)-2
...
if isinf(mse_desc)&isinf(mse_asc)
